    ...
    for category_name, test_pattern in test_categories:
        ...
        try:
            ...
            if category_results["passed"]:
                ...
                print(
                    f"✅ {category_name}: PASSED ({category_results['tests']['passed']}/{category_results['tests']['total']} tests)"
                )
            else:
                results["overall"]["failed_categories"] += 1
                print(
                    f"❌ {category_name}: FAILED ({category_results['tests']['failed']}/{category_results['tests']['total']} tests failed)"
                )
...
def generate_comprehensive_report(results: dict[str, Any], output_dir: Path) -> Path:
    ...
    success_rate = (
        (overall["passed_tests"] / overall["total_tests"] * 100)
    ...
    # Category breakdown
    report_lines.extend(["## Test Category Results", ""])
        ...
        for category in failed_categories:
            ...
            report_lines.extend(
                [
                    ...
                    "- Compare with SNP_Primer_Pipeline2/bin/ source code",
                    "- Focus on algorithm consistency and parameter matching",
                    "",
                ]
            )
...
def generate_json_summary(results: dict[str, Any], output_dir: Path) -> Path:
    ...
    summary = {
        ...
        "categories": {
            pattern: {"name": cat["name"], "passed": cat["passed"], "tests": cat["tests"]}
...
def main():
    ...
    parser.add_argument(
        "--json-only", action="store_true", help="Generate only JSON summary (skip markdown report)"
    )
