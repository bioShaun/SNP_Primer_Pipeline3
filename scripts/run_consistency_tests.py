...
def main():
    ...
    parser.add_argument(
        "-k", "--pattern", default="*", help="Test pattern to match (default: run all tests)"
    )
    ...
    parser.add_argument(
        "--parser-only", action="store_true", help="Run only parser consistency tests"
    )

    parser.add_argument(
        "--blast-only", action="store_true", help="Run only BLAST consistency tests"
    )
    ...
    success = run_consistency_tests(
        test_pattern=test_pattern, verbose=args.verbose, generate_report=not args.no_report
    )
