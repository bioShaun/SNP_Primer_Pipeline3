...
class CAPSDesigner:
            ...
            with open(self.enzyme_file) as f:
                for raw_line in f:
                    ...
                    enzyme = RestrictionEnzyme(
                        name=enzyme_name, sequence=sequence.upper(), price=price
                    )
    ...
    def find_usable_enzymes(
        ...
        wild_seq = (
            template_sequence[:snp_position] + allele_a + template_sequence[snp_position + 1 :]
        ).lower()

        mut_seq = (
            template_sequence[:snp_position] + allele_b + template_sequence[snp_position + 1 :]
        ).lower()
    ...
    def _test_enzyme(
        self, enzyme: RestrictionEnzyme, wild_seq: str, mut_seq: str, snp_position: int
    ) -> RestrictionEnzyme:
        """Test if enzyme can be used for CAPS or dCAPS."""
        # Create a copy to avoid modifying original
        test_enzyme = RestrictionEnzyme(
            name=enzyme.name, sequence=enzyme.sequence, price=enzyme.price
        )
    ...
    def _check_dcaps(
        ...
        for i in range(len(enzyme_seq)):
            ...
            pattern = self._seq_to_pattern(enzyme_seq[:i] + "N" + enzyme_seq[i + 1 :])
        ...
        if enzyme.dcaps:
            ...
            for primer_end_pos in enzyme.primer_end_positions:
                for var_site in variant_sites:
                    ...
                    # Check product size
                    if (
                        right_end - left_end < product_min - 35
                        or right_end - left_end > product_max - 35
                    ):
                        continue
        ...
        try:
            with open(output_file, "w") as f:
                ...
                for enzyme in dcaps_enzymes:
                    ...
                    f.write(
                        f"{enzyme.name}\t{enzyme.sequence}\t{enzyme.change_pos}\t{positions}\t{potential_primer}\n"
                    )
