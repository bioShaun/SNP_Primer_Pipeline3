...
def _get_homeo_seq(fasta: dict, target: str, ids: list, align_left: int, align_right: int) -> list:
    """
...
class MultipleSequenceAlignment:
    ...
    def template_to_alignment(self, template_pos: int) -> int | None:
        """
    ...
    def alignment_to_template(self, align_pos: int) -> int | None:
        """
    ...
    def find_variant_sites(
        ...
        gap_left = max(
            [len(seq.sequence) - len(seq.sequence.lstrip("-")) for seq in self.sequences]
        )
    ...
    def _is_gap_free_region(
        self, sequence: AlignedSequence, center_pos: int, left_margin: int, right_margin: int
    ) -> bool:
    ...
    def get_sequence_by_name(self, name: str) -> AlignedSequence | None:
...
class MultipleSequenceAligner:
    ...
    def _run_muscle(self, input_file: Path, output_file: Path) -> None:
        ...
        cmd = [
            str(self.software_paths.muscle_path),
            "-in",
            str(input_file),
            "-out",
            str(output_file),
        ...
        try:
            ...
            raise AlignmentError(
                f"MUSCLE not found at {self.software_paths.muscle_path}: {e}"
            ) from e
