...
class BlastRunner:
    ...
    def run(self, query_file: Path, output_file: Path) -> Path:
        ...
        cmd = [
            ...
            str(output_file),
            "-outfmt",
            "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq sseq slen",
            "-word_size",
            "11",  # V2 compatibility - important for finding all homeologs
            "-num_threads",
            str(self.threads),
            "-max_target_seqs",
            "20",
...
class FlankingExtractor:
    ...
    def extract_flanking_regions(
        ...
        for query_id, hits in blast_hits.items():
            ...
            for hit in hits:
                ...
                subject_snp_pos = self._calculate_subject_snp_position(hit, snp_pos_1based)
        ...
        for query_id in snp_hit_counts:
            if (
                snp_hit_counts[query_id] <= max_hits
    ...
    def _parse_query_id(
        self, query_id: str, known_snp_names: list[str] | None = None
    ) -> tuple[str, str, str]:
    ...
    def _calculate_subject_snp_position(self, hit: BlastHit, snp_pos: int) -> int | None:
        """
    ...
    def _create_flanking_region(
        self, snp_name: str, hit: BlastHit, subject_snp_pos: int, flanking_size: int, allele: str
    ) -> FlankingRegion:
            ...
            with open(output_file, "w") as f:
                for region in regions:
                    ...
                    cmd = [
                        ...
                        "-strand",
                        "plus" if region.strand == Strand.PLUS else "minus",
                    ]
