...
class TestPipelineSmoke:
    ...
    def test_complete_kasp_pipeline(
        self,
        reference_kasp_data,
        reference_polymarker_input,
        test_reference_db,
        comparator,
        reporter,
        tmp_path,
        ...
        if failed:
            pytest.fail(
                f"Complete KASP pipeline failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_complete_caps_pipeline(
        self,
        reference_caps_data,
        reference_polymarker_input,
        test_reference_db,
        ...
        if failed:
            pytest.fail(
                f"Complete CAPS pipeline failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_intermediate_files_consistency(
        self, reference_polymarker_input, test_reference_db, comparator, reporter, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Intermediate files consistency failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_variation_sites_list_consistency(
        self, reference_loader, snp_names, test_reference_db, comparator, reporter, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Variation sites list consistency failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_pipeline_error_handling(self, test_reference_db, comparator, reporter, tmp_path):
        ...
        if failed:
            pytest.fail(
                f"Pipeline error handling failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
