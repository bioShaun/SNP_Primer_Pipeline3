...
class TestFlankingConsistency:
    ...
    def test_flanking_region_coordinates(
        self, reference_blast_data, snp_names, comparator, reporter, test_reference_db, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Flanking coordinate calculation failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_snp_position_calculation(
        self, reference_blast_data, snp_names, comparator, reporter, test_reference_db
    ):
        ...
        if failed:
            pytest.fail(
                f"SNP position calculation failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_sequence_content_consistency(self, reference_loader, snp_names, comparator, reporter):
        ...
        if failed:
            pytest.fail(
                f"Flanking sequence content validation failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_hit_filtering_consistency(
        self, reference_blast_data, snp_names, comparator, reporter, test_reference_db
    ):
        ...
        if failed:
            pytest.fail(
                f"Hit filtering failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_gap_handling(
        ...
        if failed:
            pytest.fail(
                f"Gap handling failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
