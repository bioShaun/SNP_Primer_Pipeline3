...
class TestAlignmentConsistency:
    ...
    def test_alignment_output_consistency(
        self, reference_loader, snp_names, comparator, reporter, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Alignment output consistency failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_variation_sites_diff_all(
        self, reference_loader, snp_names, comparator, reporter, tmp_path
    ):
                ...
                try:
                    ...
                    for seq_name, seq_content in reference_alignment.items():
                        aligned_sequences.append(
                            AlignedSequence(name=seq_name, sequence=seq_content)
                        )
        ...
        if failed:
            pytest.fail(
                f"Variation sites (diff all) failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_variation_sites_diff_any(
        self, reference_loader, snp_names, comparator, reporter, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Variation sites (diff any) failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_alignment_quality_metrics(self, reference_loader, snp_names, comparator, reporter):
        ...
        if failed:
            pytest.fail(
                f"Alignment quality validation failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_coordinate_mapping(self, reference_loader, snp_names, comparator, reporter):
        ...
        if failed:
            pytest.fail(
                f"Coordinate mapping failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_homeolog_sequence_extraction(self, reference_loader, snp_names, comparator, reporter):
                    ...
                    results.extend(
                        [
                            ...
                            ComparisonResult(
                                ...
                                actual_value="reasonable similarity"
                                if similarity_ok
                                else "low similarity",
        ...
        if failed:
            pytest.fail(
                f"Homeolog validation failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
