...
class TestParserConsistency:
    ...
    def test_snp_name_extraction(self, reference_polymarker_input, comparator, reporter, tmp_path):
        ...
        if failed:
            pytest.fail(
                f"SNP name extraction failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )

    def test_chromosome_extraction(
        self, reference_polymarker_input, comparator, reporter, tmp_path
    ):
        ...
        if failed:
            pytest.fail(
                f"Chromosome extraction failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_iupac_conversion(self, reference_polymarker_input, comparator, reporter, tmp_path):
        ...
        for snp_notation, expected_iupac in expected_conversions.items():
            ...
            test_input_file = (
                tmp_path
                / f"test_iupac_{snp_notation.replace('/', '_').replace('[', '').replace(']', '')}.csv"
            )
            with open(test_input_file, "w") as f:
        ...
        if failed:
            pytest.fail(
                f"IUPAC conversion failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_fasta_output_format(self, reference_polymarker_input, comparator, reporter, tmp_path):
        ...
        with open(fasta_output_file) as f:
        ...
        if failed:
            pytest.fail(
                f"FASTA output format failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_sequence_processing(self, reference_polymarker_input, comparator, reporter, tmp_path):
        ...
        if failed:
            pytest.fail(
                f"Sequence processing failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_error_handling(self, comparator, reporter, tmp_path):
        ...
        if failed:
            pytest.fail(
                f"Error handling failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
