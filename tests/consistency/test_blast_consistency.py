...
class TestBlastConsistency:
    ...
    def test_blast_hit_count(
        self,
        reference_blast_data,
        comparator,
        reporter,
        test_input_file,
        test_reference_db,
        ...
        try:
            from snp_primer_pipeline.core.blast import BlastParser, BlastRunner
        except ImportError:
        ...
        if failed:
            pytest.fail(
                f"BLAST basic functionality failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_subject_id_consistency(self, reference_blast_data, comparator, reporter):
        ...
        for query_id, hits in reference_blast_data.items():
            ...
            for subject_id in unique_subjects:
                ...
                is_valid_subject = any(
                    pattern in subject_id.lower() for pattern in ["chr", "scaffold", "contig"]
                )
        ...
        if failed:
            pytest.fail(
                f"Subject ID consistency failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_alignment_coordinates(self, reference_blast_data, comparator, reporter):
        ...
        for query_id, hits in reference_blast_data.items():
            for i, hit in enumerate(hits):
                ...
                subject_valid = (
                    1 <= hit.subject_start <= hit.subject_end
                    or 1 <= hit.subject_end <= hit.subject_start
                )
                results.append(
                    ComparisonResult(
                        field_name=f"{hit_prefix} subject coordinates",
                        expected_value="valid coordinates",
                ...
                length_reasonable = (
                    hit.alignment_length <= query_span + 50
                    and hit.alignment_length >= query_span - 50
                )
        ...
        if failed:
            pytest.fail(
                f"Alignment coordinates failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_scores_and_evalues(self, reference_blast_data, comparator, reporter):
        ...
        if failed:
            pytest.fail(
                f"Scores and e-values failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_blast_output_parsing(self, reference_blast_data, comparator, reporter, examples_dir):
        ...
        if failed:
            pytest.fail(
                f"BLAST output parsing failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
    ...
    def test_blast_filtering(self, reference_blast_data, comparator, reporter):
        ...
        if failed:
            pytest.fail(
                f"BLAST filtering failed for {len(failed)} cases: {[f.field_name for f in failed[:3]]}"
            )
