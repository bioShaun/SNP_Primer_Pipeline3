    ...
    def __post_init__(self):
        if not self.message:
            if self.is_match:
                ...
                self.message = (
                    f"✗ {self.field_name}: expected {self.expected_value}, got {self.actual_value}"
                )
...
class OutputComparator:
    ...
    def compare_kasp_primers(
        self, expected: list[KASPPrimerRecord], actual: list[KASPPrimerRecord], snp_name: str = ""
    ...
    def compare_caps_primers(
        self, expected: list[CAPSPrimerRecord], actual: list[CAPSPrimerRecord], snp_name: str = ""
    ) -> list[ComparisonResult]:
    ...
    def compare_blast_results(
        self, expected: list[BlastHitRecord], actual: list[BlastHitRecord], query_id: str = ""
    ) -> list[ComparisonResult]:
        ...
        expected_sorted = sorted(
            expected, key=lambda x: (x.subject_id, x.subject_start, x.subject_end)
        )
    ...
    def compare_alignments(
        self, expected: dict[str, str], actual: dict[str, str], snp_name: str = ""
    ) -> list[ComparisonResult]:
        ...
        for name in sorted(common_names):
            results.append(
                self.compare_sequences(
                    expected=expected[name], actual=actual[name], name=f"Alignment sequence {name}"
                )
            )
    ...
    def compare_variation_sites(
        self, expected: list[int], actual: list[int], snp_name: str = ""
    ) -> list[ComparisonResult]:
    ...
    def compare_sequences(self, expected: str, actual: str, name: str) -> ComparisonResult:
    ...
    def compare_numeric(
        self, expected: int | float, actual: int | float, field_name: str, tolerance: float
    ) -> ComparisonResult:
