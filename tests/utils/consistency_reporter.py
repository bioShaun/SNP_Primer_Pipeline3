...
class ConsistencyReporter:
    ...
    def generate_report(self) -> str:
        ...
        if self.test_sections:
            report_lines.extend(["## Test Section Results", ""])
        ...
        if failed_results:
            ...
            for category, failures in failure_categories.items():
                report_lines.extend([f"### {category} ({len(failures)} failures)", ""])
                ...
                if len(failures) > 20:
                    report_lines.append(
                        f"*... and {len(failures) - 20} more {category.lower()} failures*"
                    )
                    report_lines.append("")
    ...
    def _categorize_failures(
        self, failures: list[ComparisonResult]
        ...
        for failure in failures:
            ...
            elif failure.tolerance is not None or any(
                x in field_name for x in ["tm", "gc", "score", "penalty", "evalue"]
            ):
    ...
    def _generate_recommendations(self, failures: list[ComparisonResult]) -> list[str]:
        ...
        if not recommendations:
            recommendations.append(
                "Review all modules systematically to identify the source of inconsistencies"
            )
