...
def run_v3_full_pipeline(input_file: Path, reference_db: Path, output_dir: Path) -> bool:
    ...
    from snp_primer_pipeline.core.blast import BlastParser, BlastRunner, FlankingExtractor
    ...
    try:
        ...
        for snp in snps:
            print(
                f"    - {snp.name}: {snp.chromosome}, pos={snp.snp_position}, alleles={snp.allele_a}/{snp.allele_b}"
            )
        ...
        v2_blast_output = (
            Path(__file__).parent.parent.parent / "test_data" / "v2_examples" / "blast_out.txt"
        )
        ...
        flanking_regions = flanking_extractor.extract_flanking_regions(
            blast_hits, snp_positions, flanking_size=500, max_hits=6
        )
        ...
        with open(combined_output, "w") as out:
            ...
            for snp_name, results in all_kasp_results.items():
                if results["output_file"].exists():
                    with open(results["output_file"]) as f:
                        for line in f:
    ...
    for snp_name in v2_snps & v3_snps:
        ...
        if len(common_seqs) > 0:
            ...
            if partial_matches:
                ...
                print("  ❌ No matching primer sequences")
                failed += 1
        ...
        if v2_site_list and v3_site_list:
            ...
            print(
                f"  Variation sites: V2={len(v2_site_list)}, V3={len(v3_site_list)}, common={len(common_sites)}"
            )
