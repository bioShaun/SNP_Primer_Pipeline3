...
def compare_kasp_outputs(
    v2_primers: dict[str, list[KASPPrimer]],
    ...
    for snp_name in v2_snps & v3_snps:
        ...
        if len(v2_list) != len(v3_list):
            errors.append(
                f"{snp_name}: Different primer count (V2={len(v2_list)}, V3={len(v3_list)})"
            )
            failed += 1
            continue
...
def compare_variation_sites(
    v2_sites: dict[str, list[int]], v3_sites: dict[str, list[int]]
) -> tuple[int, int, list[str]]:
    ...
    for snp_name in v2_sites:
        ...
        if v2_set == v3_set:
            ...
            extra = v3_set - v2_set
            errors.append(
                f"{snp_name}: Variation sites mismatch (missing={len(missing)}, extra={len(extra)})"
            )
            failed += 1
...
def load_v2_alignment(
    alignment_file: Path, target_name: str, snp_site: int = 501
) -> tuple[Optional["MultipleSequenceAlignment"], list[int], list[int]]:
    """
        ...
        for line in f:
            ...
            if line.startswith(">"):
                if current_name:
                    ...
                    sequences.append(AlignedSequence(name=current_name, sequence=seq_str))
    ...
    if target is None:
        ...
        if target_name in fasta:
            target = target_name
            ids = [n for n in fasta if n != target]
        else:
...
def run_v3_kasp_design(
    ...
    snp_position: int,
    output_dir: Path,
    v2_alignment_file: Path | None = None,
) -> Path | None:
    ...
    try:
        ...
        if v2_alignment_file and v2_alignment_file.exists():
            ...
            alignment, sites_diff_all, sites_diff_any = load_v2_alignment(
                v2_alignment_file, target_name
            )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for snp_name in v2_kasp_primers:
            ...
            if snp_name in v3_results:
                ...
                if len(v3_primers) > 0:
                    print("  ✅ V3 generated primers successfully")
                    total_passed += 1
                else:
                    print("  ❌ V3 failed to generate primers")
                    total_failed += 1
                ...
                if len(common_seqs) > 0:
                    ...
                    print("  Sample matching sequences:")
                    for seq in list(common_seqs)[:3]:
                        print(f"    - {seq}")
                else:
                    print("  ⚠️ No exact sequence matches (may differ in case or 3' modification)")
                ...
                print("  ❌ V3 did not process this SNP")
                total_failed += 1
        ...
        print("\n✅ V2 Reference Data:")
        print(
            f"   - KASP primers: {sum(len(p) for p in v2_kasp_primers.values())} primers for {len(v2_kasp_primers)} SNPs"
        )
        print(
            f"   - Variation sites: {sum(len(s) for s in v2_variation_sites.values())} sites for {len(v2_variation_sites)} SNPs"
        )
